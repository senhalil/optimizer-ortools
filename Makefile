...
tsp_simple.o: tsp_simple.cc $(OR_TOOLS_SOURCES)/constraint_solver/routing.h ortools_vrp.pb.h \
	ortools_result.pb.h $(TUTORIAL)/routing_common/routing_common.h tsptw_data_dt.h limits.h
...
tsp_simple: $(ROUTING_DEPS) tsp_simple.o ortools_vrp.pb.o ortools_result.pb.o $(OR_TOOLS_TOP)/lib/libortools.so
	...
	-L $(OR_TOOLS_TOP)/lib -Wl,-rpath $(OR_TOOLS_TOP)/lib -lcvrptw_lib -ldimacs -lortools \
	-L $(OR_TOOLS_TOP)/dependencies/install/lib -lprotobuf -lgflags -lglog -lCbcSolver -lCbc -lClp -lOsiClp -lCoinUtils -lCgl -lOsi \
	-o tsp_simple
